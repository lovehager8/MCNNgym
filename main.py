...
def evaluate(board):
  ...
  s = sum(values[f"{board.piece_at(i)}"] for i in range(64)) + (2*board.turn -1)*0.1*len(list(board.legal_moves))
  #return 0.5*(abs(s+10) - abs(s-10))
  #return s
  #return 20*math.atan(s)/math.pi
  return 20/(1+2**(-s/3)) - 10
...
def playbot(board, searchtime, playercolour):
  ...
  while not board.is_game_over():
    ...
    if board.turn == playercolour:
      ...
      message = f"bot played {board.san(botmove)}"
      board.push(botmove)
      print(board)
      print("")
      print(message)
